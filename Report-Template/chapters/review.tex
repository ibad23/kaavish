...
\section{Design System Synchronization Approaches}

While design-to-code generation focuses on the initial translation of designs into code, a separate but equally important problem is keeping generated components synchronized with evolving designs. Several approaches were surveyed during this project before settling on the chosen pipeline.

\subsection{Third-Party Bidirectional Sync Plugins}
Several third-party Figma plugins advertise bidirectional sync between design files and source repositories. These plugins are predominantly indie projects with paid tiers, intermittent maintenance, and no formal SLAs, raising concerns about reliability for production-grade workflows. Their reliance on proprietary cloud services also introduces vendor lock-in.

\subsection{Open-Source Sync Repositories}
Earlier open-source attempts such as \texttt{happy-ui-sync} and \texttt{figma-sync}~\cite{happyUiSync} explored syncing Figma artifacts to component code. These projects are largely unmaintained and predate the current Figma plugin API, the design-tokens specification, and modern Angular tooling, making them unsuitable for adoption in a contemporary production codebase.

\subsection{Deprecated Design System Platforms}
Platforms such as Backlight~\cite{backlight} and Specify~\cite{specify} previously offered structured synchronization between design systems and code repositories. Backlight has since been discontinued, and Specify's product focus has shifted toward token distribution rather than full sync, limiting their applicability for component-level synchronization.

\subsection{Figma MCP with Structured Workflow (Selected Approach)}
The chosen approach combines Figma's Model Context Protocol (MCP)~\cite{mcp} server with a structured, governed sync workflow. The MCP server exposes Figma metadata in real time to a Claude-based agent, while a project-specific mapping file (\texttt{design\_system\_mapping.json}), a CLI driver script, and an explicit phase plan provide automation, approval gates, design-token enforcement, Storybook visual validation, and pull-request creation. This combination matches the Knoccs roadmap requirements at a fraction of the cost of Code Connect and avoids the maintenance risks associated with third-party plugins or unmaintained repositories. The full design and evaluation of this pipeline are presented in Chapter~\ref{chap:results}.
...
\begin{figure}[ht!]
    ...
    \caption{Overall performance of major LLMs in CRMArena. Taken from \cite{Huang}.}
...
\begin{figure}[h!]
    ...
    \caption{Cost, Tokens, and Turns across top-performing agents. Taken from \cite{Huang}.}
...
While the aforementioned approaches to design-to-code conversion and RAG-based systems have been extensively documented in research and industry, Spursol has not yet implemented such solutions for its Knoccs platform. 
\\\\
While the technologies and methodologies we are implementing have been proven effective in prior work, this project represents their first application within Spursol's Knoccs ecosystem. Our final year project focuses on adapting and integrating these established techniques to meet Spursol's specific requirements and workflow.
