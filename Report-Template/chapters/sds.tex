...
\subsubsection{LLM Code Generation Agents (Claude)}
...
\subsubsection{Component Mapping Layer (\texttt{design\_system\_mapping.json})}

\textbf{Purpose}: Provide a single source of truth that links every Figma component frame to its Angular implementation and Storybook story, anchoring both the initial generation and the ongoing sync pipeline.
\\\\
\textbf{Responsibilities}
\begin{itemize}
    \item Map Figma component nodes $\leftrightarrow$ Angular component paths $\leftrightarrow$ Storybook stories.
    \item Track the last-synced timestamp per component to surface staleness.
    \item Serve as the input artifact consumed by \texttt{sync-figma.js} during sync runs.
\end{itemize}
...
\begin{itemize}
    \item Create Tags, color/token mapping, and preview chips.
    \item Assign tags to emails/conversations.
\end{itemize}
...
\begin{itemize}
    \item \texttt{GET /api/tags/modules/\{clientId\}/\{moduleId\}} \\ Retrieves all tags for a given client; the \texttt{moduleId} dimension was not implemented as tags are scoped only at the client level.
...
\begin{itemize}
    ...
    \item Call the LLM with retrieved context and return answer with citations.
\end{itemize}
...
\textbf{Responsibilities}
\begin{itemize}
    \item Persist relational data (companies, external\_employees, internal\_employees).
    \item Persist embeddings in vector columns with indexes.
    \item Main tables: companies, external\_employees, internal\_employees, chunks, documents, embeddings (vector)
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Semantic Retrieval:} The system queries the Vector DB using the user's input vector. It performs a similarity search (e.g., Cosine Similarity) and uses techniques like BM-25 and Reranker to retrieve the "Best Search Results"—the most relevant data chunks ingested during the first stage.
    
    \item \textbf{Context Assembly:} The system constructs a composite prompt in the "Preparing the Model Input" step. This package includes:
