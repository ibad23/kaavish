...
\subsection{Knowledge Base (RAG)}
Figure \ref{fig:rag_usecase} illustrates the Knowledge Base use cases. The \textbf{Data Synchronization Agent} triggers the embedding generation process, converting documents into vector representations stored in the database. The \textbf{User} submits natural-language queries and views the results along with source citations. The \textbf{AI Model} retrieves similar chunks from the vector store and generates grounded answers based on the retrieved context.
