...
\begin{itemize}
    ...
    The system must connect to Figma via MCP to fetch design files, layers, and tokens automatically. This integration ensures real-time design synchronization and eliminates manual file handling.
    \item \textbf{Design Token Extraction:}
    The system should extract and standardize color, typography, spacing, and other design tokens into a standardized \texttt{SCSS} file for consistent design implementation.
...
\begin{itemize}
    \item \textbf{Description:} Create, and assign tags across modules with color tokens.
    \item The system must allow users to create tags.
    ...
    \item The system must ensure tags are searchable within each module.
\end{itemize}
...
\begin{itemize}
    ...
    \item The system must allow documentation and product data from different sources (e.g., SharePoint, databases, local files, etc.)
...
\begin{itemize}
    \item \textbf{Pipeline Performance (Single Component):} The end-to-end translation of a single component (from Figma publish to committed Angular code) should complete in under 300 seconds.
...
\begin{itemize}
    \item \textbf{Component Scalability:} The pipeline must be able to support a design system growing to 50+ unique components and variants.
    ...
    \item \textbf{Governance:} The system must include an approval workflow for all design-to-code component mappings to ensure quality, traceability, and consistency before merging into the main branch.
\end{itemize}
...
\subsubsection{Tag Management}
\begin{itemize}
    \item \textbf{Performance:} Applying a tag must be reflected in the UI in under 500ms. Searching by tag must return results in under 2 seconds.
...
\\\\
Figure \ref{fig:tag_srs} shows the Tag Management interface. The top section provides a search bar for filtering tags, alongside a ``Create Tag'' button for administrators. Tags are displayed as cards in a grid layout, each showing the tag name, a color-coded chip preview, the number of entities using the tag, and action icons for editing, duplicating, and deleting. 
\begin{figure}[H]
    \centering
    \begin{tabular}{cc}
        \includegraphics[width=0.5\textwidth]{figures/srs/tag1.png} &
        \includegraphics[width=0.5\textwidth]{figures/srs/tag2.png} 
    \end{tabular}
    \caption{Tag management - (a) Interface - Part 1 \& (b) Interface - Part 2}
    \label{fig:tag_srs}
\end{figure}

\textbf{Macro Management}
\\\\
Figure \ref{fig:macro} presents the Macro Management interface in two parts. Part (a) shows the macro list view with a search bar, filter tabs (``All Triggers'' and ``Most Used''), and a ``Create Macro'' button. Part (b) shows the lower section of the same view, featuring a ``Visual Macro Composer'' section that provides a form-based interface for building custom macros with predefined actions and conditions. 
...
\begin{figure}[H]
    ...
    \end{tabular}
    \caption{Macro management - (a) Interface - Part 1 \& (b) Interface - Part 2}
    \label{fig:macro}
\end{figure}

\textbf{AI usecase: RAG chatbot}
\\\\
Figure \ref{fig:rag} shows the RAG Document Assistant interface integrated within the Knoccs sidebar. Users interact through a conversational chat interface where they can submit natural-language queries and receive AI-generated responses grounded in the knowledge base. Responses are rendered in Markdown format with structured information such as numbered feature lists, and users can ask follow-up questions within the same conversation context.
...
This section presents detailed use cases of our system. The following actors are involved across the use case diagrams:

\begin{itemize}
    \item \textbf{Designer:} A UI/UX professional responsible for creating and maintaining the component design system in Figma, annotating components for code generation, and reviewing generated code for visual fidelity.
    \item \textbf{Developer:} A software engineer who reviews the AI-generated Angular code, approves final implementations, and integrates them into the Knoccs codebase.
    \item \textbf{Figma:} The external design tool that serves as the source of truth for design specifications, accessed programmatically via its MCP Server.
    \item \textbf{AI Agent:} The Claude Code agent that fetches design metadata, generates Angular components (TypeScript, HTML, SCSS), and orchestrates the code generation pipeline.
    \item \textbf{User:} An end user of the Knoccs platform who interacts with the Knowledge Base, assigns tags, and executes macros.
    \item \textbf{Admin:} An administrator with elevated permissions for creating, editing, and deleting tags, macros, and managing platform configurations.
    \item \textbf{AI Model:} The LLM that generates answers from retrieved context in the RAG pipeline.
    %\item \textbf{Data Synchronization Agent:} The backend agent responsible for triggering document embedding, managing metadata, and keeping the vector store updated.
    % \item \textbf{Triggered Event:} An automated system event that initiates macro auto-execution based on predefined conditions.
\end{itemize}

\newpage

\subsection{Design System}
Figure \ref{fig:design_usecase} illustrates the Design-to-Code System use cases. The \textbf{Designer} annotates components in Figma with mapping hints and triggers code generation. The \textbf{AI Agent} fetches Figma files via the MCP Server, maps design elements to Angular components, and generates the corresponding TypeScript, HTML, and SCSS files. The \textbf{Developer} reviews the generated code and approves the final implementation before it is merged into the codebase. \textbf{Figma} acts as an external actor providing design metadata through the MCP protocol.
...
\subsubsection{Tag Management}
Figure \ref{fig:tag_usecase} illustrates the Tag Management use cases. The \textbf{User} can search tags, filter entities by tags, and assign or unassign tags to entities such as conversations and contacts. The \textbf{Admin} has elevated permissions to create new tags with color tokens, edit existing tag properties (name, color), and delete tags from the system. 
...
\subsubsection{Macro Management}
Figure \ref{fig:macro_usecase} illustrates the Macro Management use cases. The \textbf{User} can execute macros manually and view macro execution results. The \textbf{Admin} can create new macros through the Visual Macro Composer, edit existing macros, and delete macros from the system. The \textbf{Triggered Event} actor represents automated system events that initiate macro auto-execution based on predefined schedules or conditions, enabling workflow automation without manual intervention.
...
\subsection{Knowledge Base (RAG)}
Figure \ref{fig:rag_usecase} illustrates the Knowledge Base use cases. The \textbf{Data Synchronization Pipeline} triggers the embedding generation process, converting documents into vector representations stored in the database. The \textbf{User} submits natural-language queries and views the results along with source citations. The \textbf{AI Model} retrieves similar chunks from the vector store and generates grounded answers based on the retrieved context.
...
\begin{itemize}
    ...
    \item \textbf{Figma-to-Code Sync Pipeline} monitors for design changes in Figma and propagates targeted updates to the corresponding Angular component code. It operates through an 8-phase workflow triggered via a CLI script and executed by Claude Code, keeping the component library aligned with evolving designs at 80-90\% automation.
...
\begin{itemize}
    ...
    \item \textbf{Figma-to-Code Sync Pipeline} reads the component mapping configuration, detects differences between Figma designs and existing code, and applies targeted updates through Claude Code. The sync is initiated via a CLI command and follows an 8-phase workflow with human approval at each gate
\end{itemize}
