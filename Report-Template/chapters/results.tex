...
The development environment consisted of the Angular 16 frontend of the Knoccs application, with components generated into \texttt{src/app/shared/components/}. Storybook served as the visual documentation and validation layer. Each generated component produced four to five files: \texttt{.component.ts}, \texttt{.component.html}, \texttt{.component.scss}, \texttt{.types.ts}, and optionally \texttt{.interfaces.ts}.

For the sync workflow, three additional artifacts were configured, \texttt{sync-figma.js} (a Node.js driver invoked through \texttt{npm run sync:figma <component>}), and an 8-phase pipeline Claude follows during a sync run. These artifacts make the sync pipeline reproducible across the team and remove the need to hand-author prompts for individual sync runs. The sync interface, described in Section~\ref{sec:sync}, sits on top of these artifacts and exposes them through a designer-facing interface.

\subsection{Iterative Instruction Refinement}
\label{sec:instruction-engineering}
...
% A mandatory agent chain was defined: \\ \texttt{figma-extractor} $\rightarrow$ \texttt{component-generator} $\rightarrow$ \texttt{storybook-generator} $\rightarrow$ \texttt{unit-test-generator} $\rightarrow$ \texttt{claude-md-compliance-checker}. 
This version also introduced the complete design token system (color scales from \texttt{\$color-0} through \texttt{\$color-100} across 8 palettes), the \texttt{toRem()} function for responsive pixel-to-rem conversion, BEM-like CSS naming conventions, icon system documentation, and typography specifications.
...
Accordion & 0.8297 & Phone Input & 0.8760 \\ \hline
Avatar & 0.9615 & Popup & 0.8376 \\ \hline
Breadcrumb & 0.9704 & Radio Button & 0.9576 \\ \hline
Button (Success) & 0.8557 & Single-Select Dropdown & 0.9310 \\ \hline
Button (Warning) & 0.8433 & Icon & 0.8610 \\ \hline
Checkbox & 0.7013 & Snackbar & 0.5240 \\ \hline
Chip & 0.9370 & Stepper & 0.9702 \\ \hline
Datepicker & 0.9165 & Tabs & 0.8188 \\ \hline
Input & 0.9400 & Toastbar & 0.9573 \\ \hline
List & 0.9185 & Toggle & 0.9080 \\ \hline
Multi-Select Dropdown & 0.8997 & Tooltip & 0.8883 \\ \hline
\multicolumn{4}{c}{} \\ \hline
\textbf{Average} & \textbf{0. 8946} & \textbf{Median} & \textbf{0.847} \\ \hline
...
\begin{table}[H]
\centering
\caption{DeepEval RAG Evaluation Results}
\label{tab:deepeval_results}
\begin{tabular}{|l|c|c|}
\hline
\textbf{Metric} & \textbf{Results} \\ \hline
Answer Relevancy Evaluates & 84.7\%  \\ \hline
Faithfulness Measures & 96.2\%  \\ \hline
Contextual Precision Measures & 76.2\%  \\ \hline
\end{tabular}
\end{table}
